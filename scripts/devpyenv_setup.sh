...
then
    ...
    then
        ...
        [[ -x "$VIRTENVPATH/bin/python2" || -z "${LD_LIBRARY_PATH:-}" ]] || unset LD_LIBRARY_PATH
    else
        ...
        then
            ...
            PYTHON='/sw/packages/python/current/bin/python'
            VIRTUALENV='/sw/packages/python/current/bin/virtualenv'
        fi
