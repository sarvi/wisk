...
int main(void)
{
    ...
    char *envp[] = {"LD_PRELOAD=/nobackup/sarvi/wisktrack/src/libwisktrack.so", NULL};
    ...
    fprintf(f, MSG);
    fclose(f);
    int ret = system("export WISK_TRACKER_DEBUGLEVEL=3; export WISK_TRACKER_UUID=casaASAS; export WISK_TRACKER_PIPE=/tmp/wisk_tracker.pipe; export LD_PRELOAD=/nobackup/sarvi/wisktrack/src/libwisktrack.so; /bin/cat test_execve1.c");
